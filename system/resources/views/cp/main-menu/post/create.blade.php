...
<div class="section-header-back">
    @if($menu->parent_id == 0)
    <a href="{{route('cp.main-menus.show',$menu->id)}}" class="btn btn-icon"><i class="fas fa-arrow-left"></i></a>
    @else
    <a href="{{route('cp.main-menus.submenus.show',[$menu->parent_id,$menu->id])}}" class="btn btn-icon"><i class="fas fa-arrow-left"></i></a>
    @endif
</div>
<h1>Tambah Post</h1>
<div class="section-header-breadcrumb">
    ...
    <div class="breadcrumb-item"><a href="{{ $breadcrumb->parent_id == 0 ? route('cp.main-menus.show', $breadcrumb->id) : route('cp.main-menus.submenus.show', [$breadcrumb->parent_id, $breadcrumb->id]) }}">{{ $breadcrumb->title }}</a></div>
    @endforeach
...
@section('content')
<form action="{{ route('cp.main-menus.posts.store', $menu) }}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="row">
        <div class="col-md-8">
            <div class="card">
        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-footer bg-whitesmoke">
                    ...
                    @if($menu->parent_id == 0)
                    <a href="{{route('cp.main-menus.show',$menu->id)}}" class="btn btn-secondary">
                        Batal
                    </a>
                    @else
                    <a href="{{route('cp.main-menus.submenus.show',[$menu->parent_id,$menu->id])}}" class="btn btn-secondary">
                        Batal
                    </a>
                    @endif
                </div>
...
<div class="d-none">
    <table>
        <tbody id="table-row">
            <tr>
                <td>
                    <input type="text" class="form-control files" id="title_files" name="title_files[]" required="">
                </td>
                <td>
                ...
                <td>
                    <label class="custom-switch mt-2">
                        <input type="checkbox" name="is_downloadable[]" class="custom-switch-input" value="1">
                        <span class="custom-switch-indicator"></span>
                      </label>
                </td>
                <td>
